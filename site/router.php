...
/**
 * Routing class from com_festival
 *
 * @since  3.3
 */
class FestivalRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_festival component
	 *
	...
	public function build(&$query)
	{
		...
		$params = JComponentHelper::getParams('com_festival');
		...
		if (isset($query['view']))
		{
			...
			if (empty($query['Itemid']) && !(isset($view) && isset($query['id']) && ($view === 'project')))
			{
				$segments[] = $query['view'];
			}
		...
		if (isset($view) && isset($query['id']) && ($view === 'project'))
		{
	...
	{		
		...
		switch($segments[0])
		{
			case 'project':
				...
				{
					$id = $this->getVar('project', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'festival')
	{
...
function FestivalBuildRoute(&$query)
{
	$router = new FestivalRouter;
	
	return $router->build($query);
}

function FestivalParseRoute($segments)
{
	$router = new FestivalRouter;
