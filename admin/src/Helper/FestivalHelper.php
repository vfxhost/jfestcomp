...
namespace Acme\Festival\Administrator\Helper;
...
use PhpOffice\PhpSpreadsheet\Reader\chunkReadFilter;
...
abstract class FestivalHelper
{
    ...
    public static function addSubmenu($submenu)
    {
        ...
        Sidebar::addEntry(Text::_('COM_FESTIVAL_SUBMENU_DASHBOARD'), 'index.php?option=com_festival&view=festival', $submenu === 'festival');
        if (!$user->authorise('project.access', 'com_festival')) {
            return;
        }
        if (!$user->authorise('project.submenu', 'com_festival')) {
            return;
        }
        Sidebar::addEntry(Text::_('COM_FESTIVAL_SUBMENU_PROJECS'), 'index.php?option=com_festival&view=projecs', $submenu === 'projecs');
    }
    /**
     * Prepares the xml document
     */
    public static function xls($rows, $fileName = \null, $title = \null, $subjectTab = \null, $creator = 'Vfxhost', $description = \null, $category = \null, $keywords = \null, $modified = \null)
    {
        // set the user
        $user = Factory::getUser();
        ...
        $spreadsheet->getProperties()->setCreator($creator)->setCompany('Vfxhost')->setLastModifiedBy($modified)->setTitle($title)->setSubject($subjectTab);
        // The file type
        $file_type = 'Xls';
        // set description
        if ($description) {
        ...
        // Some styles
        $headerStyles = ['font' => ['bold' => \true, 'color' => ['rgb' => '1171A3'], 'size' => 12, 'name' => 'Verdana']];
        $sideStyles = ['font' => ['bold' => \true, 'color' => ['rgb' => '444444'], 'size' => 11, 'name' => 'Verdana']];
        $normalStyles = ['font' => ['color' => ['rgb' => '444444'], 'size' => 11, 'name' => 'Verdana']];
        // Add some data
        if (($size = self::checkArray($rows)) !== \false) {
            ...
            foreach ($rows as $array) {
                ...
                foreach ($array as $value) {
                    ...
                    if ($xls_mode != 3) {
                        if ($i == 1) {
                            $active_sheet->getColumnDimension($a)->setAutoSize(\true);
    ...
    protected static function composephpspreadsheet()
    {
        ...
        // do not load again
        self::$composer['phpspreadsheet'] = \true;
        return \true;
    }
    /**
     * Get a Variable
     ...
     */
    public static function getModel($name, $path = \JPATH_COMPONENT_ADMINISTRATOR, $Component = 'Festival', $config = [])
    {
        // fix the name
