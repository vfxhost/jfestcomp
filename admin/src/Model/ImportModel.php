...
/***
 * Festival Import Base Database Model
 */
class ImportModel extends BaseDatabaseModel
{
     ...
     * @var        string
     */
    protected $_context = 'com_festival.import';
    /**
     * Import Settings
     */
    ...
    protected function populateState()
    {
        ...
        $this->setState('message', $app->getUserState('com_festival.message'));
    ...
    public function import()
    {
        ...
        if ($package === \null) {
            switch ($this->getType) {
                ...
                case 'continue':
                    ...
                    break;
                default:
                    $app->setUserState('com_festival.message', Text::_('COM_FESTIVAL_IMPORT_NO_IMPORT_TYPE_FOUND'));
                    return \false;
                    break;
            }
        }
        ...
        if (!$package || !$package['type']) {
            if (\in_array($this->getType, ['upload', 'url'])) {
                $this->remove($package['packagename']);
            }
            $app->setUserState('com_festival.message', Text::_('COM_FESTIVAL_IMPORT_UNABLE_TO_FIND_IMPORT_PACKAGE'));
            return \false;
        }
        ...
        if (!$continue) {
            ...
            $session->set('hasPackage', \true);
            return \true;
        }
        // set the data
        $headerList = \json_decode($session->get($this->dataType . '_VDM_IMPORTHEADERS', \false), \true);
        if (!$this->setData($package, $this->dataType, $headerList)) {
            // There was an error importing the package
            $msg = Text::_('COM_FESTIVAL_IMPORT_ERROR');
            $back = $session->get('backto_VDM_IMPORT', \NULL);
            if ($back) {
                $app->setUserState('com_festival.redirect_url', 'index.php?option=com_festival&view=' . $back);
                $session->clear('backto_VDM_IMPORT');
            }
            $result = \false;
        } else {
            // Package imported sucessfully
            $msg = Text::sprintf('COM_FESTIVAL_IMPORT_SUCCESS', $package['packagename']);
            $back = $session->get('backto_VDM_IMPORT', \NULL);
            if ($back) {
                $app->setUserState('com_festival.redirect_url', 'index.php?option=com_festival&view=' . $back);
                $session->clear('backto_VDM_IMPORT');
    ...
    protected function _getPackageFromUpload()
    {
        ...
        $userfile = $input->files->get('import_package', \null, 'raw');
        // Make sure that file uploads are enabled in php
        if (!(bool) \ini_get('file_uploads')) {
            $app->enqueueMessage(Text::_('COM_FESTIVAL_IMPORT_MSG_WARNIMPORTFILE'), 'warning');
            return \false;
        }
        // If there is no uploaded file, we have a problem...
        if (!\is_array($userfile)) {
            $app->enqueueMessage(Text::_('COM_FESTIVAL_IMPORT_MSG_NO_FILE_SELECTED'), 'warning');
            return \false;
        }
        // Check if there was a problem uploading the file.
        if ($userfile['error'] || $userfile['size'] < 1) {
            $app->enqueueMessage(Text::_('COM_FESTIVAL_IMPORT_MSG_WARNIMPORTUPLOADERROR'), 'warning');
            return \false;
        }
    ...
    protected function _getPackageFromFolder()
    {
        ...
        if (!\file_exists($p_dir)) {
            $app->enqueueMessage(Text::_('COM_FESTIVAL_IMPORT_MSG_PLEASE_ENTER_A_PACKAGE_DIRECTORY'), 'warning');
            return \false;
        }
        ...
        if (!$type) {
            $app->enqueueMessage(Text::_('COM_FESTIVAL_IMPORT_MSG_PATH_DOES_NOT_HAVE_A_VALID_PACKAGE'), 'warning');
        }
        // check the extention
        if (!$this->checkExtension($p_dir)) {
            // set error message
            $app->enqueueMessage(Text::_('COM_FESTIVAL_IMPORT_MSG_DOES_NOT_HAVE_A_VALID_FILE_TYPE'), 'warning');
            return \false;
        }
        $package['packagename'] = \null;
        $package['dir'] = $p_dir;
    ...
    protected function _getPackageFromUrl()
    {
        ...
        if (!$url) {
            $app->enqueueMessage(Text::_('COM_FESTIVAL_IMPORT_MSG_ENTER_A_URL'), 'warning');
            return \false;
        }
        ...
        // Was the package downloaded?
        if (!$p_file) {
            $app->enqueueMessage(Text::_('COM_FESTIVAL_IMPORT_MSG_INVALID_URL'), 'warning');
            return \false;
        }
        // check that this is a valid spreadsheet
    ...
    {
        $data = [];
        if (FestivalHelper::checkArray($target_headers)) {
            // make sure the file is loaded
            FestivalHelper::composerAutoload('phpspreadsheet');
            $jinput = Factory::getApplication()->input;
            foreach ($target_headers as $header) {
                if (($column = $jinput->getString($header, \false)) !== \false || ($column = $jinput->getString(\strtolower($header), \false)) !== \false) {
                    $data['target_headers'][$header] = $column;
                } else {
                    $data['target_headers'][$header] = \null;
                }
            }
            // set the data
            if (isset($package['dir'])) {
                ...
                $excelReader->setReadDataOnly(\true);
                $excelObj = $excelReader->load($package['dir']);
                $data['array'] = $excelObj->getActiveSheet()->toArray(\null, \true, \true, \true);
    ...
    protected function save($data, $table)
    {
        ...
        if (FestivalHelper::checkArray($data['array'])) {
            // get user object
            ...
            if (FestivalHelper::checkArray($data['array']) && $user->authorise($table . '.import', 'com_festival') && $user->authorise('core.import', 'com_festival')) {
                // set target.
                ...
                $canDo = FestivalHelper::getActions($table);
                ...
                foreach ($data['array'] as $row) {
                    ...
                    if (isset($row[$id_key]) && \is_numeric($row[$id_key]) && $row[$id_key] > 0) {
                        ...
                        $query->select('version')->from($db->quoteName('#__festival_' . $table))->where($db->quoteName('id') . ' = ' . $db->quote($row[$id_key]));
                    ...
                    if ($found && $canEdit) {
                        ...
                        $query->update($db->quoteName('#__festival_' . $table))->set($fields)->where($conditions);
                        $db->setQuery($query);
                        $db->execute();
                    } elseif ($canCreate) {
                        // insert item
                        ...
                        $query->insert($db->quoteName('#__festival_' . $table))->columns($db->quoteName($columns))->values(\implode(',', $values));
                        ...
                        if ($done) {
                            ...
                            FestivalHelper::setAsset($aId, $table);
                        }
                    } else {
                        return \false;
                    }
    ...
    protected function getAliasesUsed($table)
    {
        ...
        $columns = $db->getTableColumns('#__festival_' . $table);
        if (isset($columns['alias'])) {
            // Create a new query object.
            $query = $db->getQuery(\true);
            $query->select($db->quoteName(['alias']));
            $query->from($db->quoteName('#__festival_' . $table));
            $db->setQuery($query);
            $db->execute();
            if ($db->getNumRows()) {
