...
namespace Acme\festival\Administrator\Service\Html;
...
class Batch_
{
    ...
    public function render()
    {
        ...
        $data = new \stdClass();
        $data->ListSelection = $this->getListSelection();
        // Create a layout object and ask it to render the batch selection options
        $layout = new FileLayout('batchselection');
     ...
     * @since   3.0
     */
    public function addListSelection($label, $name, $options, $noDefault = \false)
    {
        \array_push(static::$ListSelection, ['label' => $label, 'name' => $name, 'options' => $options, 'noDefault' => $noDefault]);
    }
    /**
